...
Read the provided `context_packet.md`, `resume_state.md` when provided, relevant discovery reports, and relevant trace reports before editing. Treat them as a map, not truth. Treat critical planning artifacts as incomplete unless your reads reach EOF or targeted searches cover the relevant sections. Independently verify the claims you rely on by inspecting the source files, tests, configs, docs, or runtime references directly.

Before editing code that imports, configures, or relies on a core library, service API, framework behavior, cloud provider feature, or package API, check the current official docs or report why that research is not applicable, unavailable, blocked, or already sufficiently covered by supplied external research. Do not invent package APIs, versions, or capabilities.
...
## External Docs / Package Checks
- Official docs checked: ...
- Public examples or package baselines checked: ...
- Package/API constraints that shaped the implementation: ...
