...
External research is part of discovery when it can materially affect architecture, package choice, implementation, validation, integration, or product risk. Check current official docs, public examples, popular open-source implementations, mature package options, and dependency ecosystem constraints when applicable. If external research is not applicable, unavailable, blocked, or forbidden by the Run Mode Contract, record an explicit skip rationale.
...
If the orchestrator provides a planning module path, context packet path, resume state path, or prior reports, read them first. Treat critical planning artifacts as incomplete unless your reads reach EOF or targeted searches cover the relevant sections. Then independently verify the claims you rely on by inspecting source files, tests, configs, docs, or runtime references directly.
...
- Official docs, public examples, open-source baselines, and package searches run: ...
...
- Mature package options considered: ...
- Dependency/API constraints found: ...
